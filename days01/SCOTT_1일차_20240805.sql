...
-- SCOTT에서 기본 SELECT문을 익히기 위해 테이블 생성.
-- 1) sql plus 실행해서 SYS 계정으로 접속.
-- 2) c:\\oraclexe 폴더   scott.sql 파일 검색
SELECT *
FROM tabs;
-- 오라클은 사용자 계정(scott)을 생성하면 동일한 이름(scott)으로 DB 객체를 생성한다.
-- => 스키마(Schema)
DESC emp; -- 테이블 구조 확인
-- (7,2) 소수점 2자리
DESCRIBE DEPTO;
...
-- DQL : SELECT문
WITH        -- 1
SELECT      -- 6 *      DISTINCT, ALL, AS 키워드 대문자 권장.
FROM        -- 2 *
WHERE       -- 3
GROUP BY    -- 4
HAVING      -- 5
ORDER BY    -- 6
-- Emp 사원 정보 조회..
FROM 스키마.테이블명 또는 스키마.뷰명
;
--
SELECT empno, ename, job, mgr, hiredate, sal, comm, deptno
SELECT * -- * 전부
FROM emp; -- scott 생략 가능.
FROM scott.emp;

-- SCOTT 소유하고 있는 테이블 정보를 조회.
SELECT *
FROM dba_tables; -- 권한이 없음, 옆집 차
FROM user_tables; -- 내 차
FROM all_tables; -- 가족 차
FROM tabs;
...
FROM emp;
-- 사원번호, 사원명, 잡 컬럼만 출력
SELECT ALL empno, ename, job -- ALL 생략돼있음.
FROM emp; -- 사원 테이블로부터 empno, ename, job 출력하겠다.

SELECT DISTINCT empno, ename, job -- 3개 다 일치해야지 중복 제거 되는 경우.
FROM emp;

-- 사원들의 속해있는 잡 컬럼만 출력(조회)
-- DISTINCT 키워드(중복 제거)
SELECT DISTINCT job
FROM emp;

SELECT COUNT (DISTINCT job) -- 숫자로
FROM emp;
--
CREATE TABLE insa(
...
INSERT INTO insa (num, name, ssn, ibsaDate, city, tel, buseo, jikwi, basicPay, sudang) VALUES
  (1060, '김신애', '810809-2111111', '2001-10-10', '서울', '011-4151-4444', '개발부', 
   '사원', 900000 , 102000);
-- DML 커밋, 롤백
COMMIT; -- DML문은 커밋 아니면 롤백을 무조건 해야된다.
