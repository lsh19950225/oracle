...
--
-- INSERT 내용 쿼리.
EXEC INS_O_REVIEW (1, 1001, '레드가 어울리는 계절이 왔어요.
레드 풀컬러에 작은 꽃송이가 올라간 핑크가 더해져 어떤 손도 화사하게 만들어 줍니다.
배치에 따라 다양한 무드로 표현할 수 있는 티니를 만나보세요!'
, SYSDATE, 5, 'Y', 'Y', '20대','데일리');

EXEC INS_O_REVIEW (1, 1002, '빨간색이 의외로 잘어울려서 구매했습니다.'
, SYSDATE, 5, 'Y', 'N', '20대','데일리');

EXEC INS_O_REVIEW (1, 1003, '가을 신상이 나왔길래 몇개 구매했는데 정말 네일 맛집답게 색상이 가을가을하니 진짜 예쁘네요.
여러 디자인중 티니 네일 제일 눈에 들어와 맨먼저 붙였어요.'
, SYSDATE, 3, 'N', 'N', '30대','자석');

EXEC INS_O_REVIEW (1, 1004, '가을 신상이 나왔길래 몇개 구매했는데 정말 네일 맛집답게 색상이 가을가을하니 진짜 예쁘네요.
여러 디자인중 티니 네일 제일 눈에 들어와 맨먼저 붙였어요.'
, SYSDATE, 4, 'N', 'N', '10대','드로잉');

EXEC INS_O_REVIEW (1, 1005, '잘 받았지만 오픈 아직 안해요ㅎㅎ'
, SYSDATE, 1, 'N', 'Y', '40대 이상','키치');
--
--
DESC o_comment;
-- 댓글 시퀀스 생성.
...
-- 
-- INSERT 댓글 쿼리.
EXEC ins_o_comment (1, 1, SYSDATE, '할머님 손톱 모양이 고우셔서 더 잘 어울리네요. 저희 친정엄마도 해드려야겠어요^^');

EXEC ins_o_comment (1, 2, SYSDATE, '할머님 손도 손녀님 마음도 너무 예뻐요!');

EXEC ins_o_comment (1, 3, SYSDATE, '너무 예쁘네요');

EXEC ins_o_comment (1, 4, SYSDATE, '저도 구입추가요^^');
